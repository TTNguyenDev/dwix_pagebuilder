...
export class MediaWorkshop {
  ...
  public async initialize(): Promise<void> {
    ...
    this.searchPattern.extend(ChangeRateLimit).subscribe(this.searchMedia);
  }
  ...
  public async searchMedia(searchPattern: string = ""): Promise<void> {
    ...
    const query = Query.from<MediaContract>().orderBy(`fileName`);
    ...
    const mediaItems = mediaOfResults.value.map(
      (media) => new MediaItem(media)
    );
  ...
  public async loadNextPage(): Promise<void> {
    ...
    const mediaItems = this.currentPage.value.map(
      (page) => new MediaItem(page)
    );
    this.mediaItems.push(...mediaItems);
  ...
  public async uploadMedia(): Promise<void> {
    this.eventManager.dispatchEvent("displayHint", {
      key: "88d9",
      content: `ajldaklsdlsakdjYou may upload pictures, videos and other files just by dropping them anywhere in editor window and even make respective widget out of them.`,
    });
    ...
    for (const file of files) {
      const uploadPromise = new Promise(async (resolve) => {
        const url: any = await IPFSUtils.uploadFileToIPFS({
          file,
        })

        const mediaContract = await this.mediaService.createMediaUrl(
          file.name,
          url,
          file.type
        );

        resolve(mediaContract)
      })

      this.viewManager.notifyProgress(
        uploadPromise,
        "Media library",
        `Uploading ${file.name}...`
      );
  ...
  public onDragEnd(item: MediaItem): void {
    ...
    if (acceptorBinding && acceptorBinding.handler) {
      const widgetHandler = this.widgetService.getWidgetHandler(
        acceptorBinding.handler
      );
