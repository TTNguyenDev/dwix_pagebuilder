...
export class GridEditor {
  ...
  private getContextCommands(
    ...
    if (context.binding?.handler) {
      const handler = this.widgetService.getWidgetHandler(
        context.binding.handler
      );
    ...
    contextualCommands.selectCommands =
      contextualCommands.selectCommands !== undefined
        ? contextualCommands.selectCommands
        : defaultCommands.selectCommands;

    contextualCommands.hoverCommands =
      contextualCommands.hoverCommands !== undefined
        ? contextualCommands.hoverCommands
        : defaultCommands.hoverCommands;

    contextualCommands.deleteCommand =
      contextualCommands.deleteCommand !== undefined
        ? contextualCommands.deleteCommand
  ...
  private onPointerDown(event: PointerEvent): void {
    if (
      event.ctrlKey ||
      event.metaKey ||
      this.viewManager.mode === ViewManagerMode.preview
    ) {
      const htmlElement = <HTMLElement>event.target;
      const htmlLinkElement = <HTMLLinkElement>htmlElement.closest("A");
      ...
      if (topGridItem.name === "content") {
        if (topGridItem.binding.model.type !== activeLayer) {
          ...
          this.eventManager.dispatchEvent(
            "displayInactiveLayoutHint",
            topGridItem.binding.model
          );
          return;
        } else {
    ...
    if (this.isSelected(gridItem)) {
      ...
      if (gridItem.element["dragSource"]) {
        // TODO: Maybe make part of Binding?
        gridItem.element["dragSource"].beginDrag(
          gridItem.element,
          this.pointerX,
          this.pointerY
        );
    ...
    switch (event.key) {
      ...
      case Keys.PageDown:
        ...
        if (gridItem.binding.model instanceof SectionModel) {
          const containerGridItem = this.getGridItem(
            <HTMLElement>gridItem.element.firstElementChild,
            true
          );
  ...
  private renderDropHandlers(): void {
    ...
    if (
      !acceptingParent ||
      dragSession.sourceParentBinding === acceptingParent.binding
    ) {
    ...
    const siblingElement: GridItem = stack.find(
      (x) => x.element.parentElement === acceptingParent.element
    );

    if (siblingElement) {
      const quadrant = Utils.pointerToClientQuadrant(
        this.pointerX,
        this.pointerY,
        siblingElement.element
      );
      const sourceElementFlow =
        dragSession.sourceBinding.flow || ComponentFlow.Inline;

      dragSession.insertIndex = acceptingParent.binding.model.widgets.indexOf(
        siblingElement.binding.model
      );
      const hoveredElementFlow =
        siblingElement.binding.flow || ComponentFlow.Inline;

      if (
        sourceElementFlow === ComponentFlow.Inline &&
        hoveredElementFlow === ComponentFlow.Inline
      ) {
      ...
      if (acceptingParent.binding.model.widgets.length === 0) {
        ...
        const children = Array.prototype.slice.call(
          acceptingParent.element.children
        );
  ...
  private onDelete(): void {
    if (
      this.viewManager.mode === ViewManagerMode.selected &&
      this.activeContextualCommands &&
      this.activeContextualCommands.deleteCommand
    ) {
  ...
  private resetScrolling(): void {
    ...
    if (this.selection) {
      // also, check for element existence.
      this.selectElement(this.selection, false);
    }
  }

  private getUnderlyingElements(): HTMLElement[] {
    const elements = Utils.elementsFromPoint(
      this.ownerDocument,
      this.pointerX,
      this.pointerY
    );

    const popupContainer = elements.find((x) =>
      x.classList.contains("popup-container")
    );
    const cutoffIndex = elements.findIndex(
      (x) =>
        x.classList.contains("backdrop") ||
        x.classList.contains("popup-backdrop") ||
        x.classList.contains("popup-container")
    );
  ...
  private getDefaultContextCommands(
    ...
    const contextCommands: IContextCommandSet = {
          ...
          component: {
            ...
            params: {
              ...
              onSelect: (newWidgetModel: any) => {
                ...
                context.parentBinding.model.widgets.splice(
                  index,
                  0,
                  newWidgetModel
                );
      ...
      selectCommands: context.binding?.editor &&
  ...
  private renderContextualCommands(): void {
    ...
    for (let i = gridItems.length - 1; i >= 0; i--) {
      ...
      if (
        !activeElement ||
        gridItem.element !== activeElement.element ||
        half !== activeElement.half
      ) {
        this.viewManager.setContextualCommands(
          gridItem.name,
          contextualCommandSet
        );
    ...
    if (
      ...
      highlightedGridItem?.name !== "content"
    ) {
      this.activeHighlightedGridItem = highlightedGridItem;
  ...
  private findFocusableElement(): GridItem {
    const element = <HTMLElement>(
      Html.findFirst(
        ...
        (node) =>
          node.nodeName === "SECTION" && !!this.getGridItem(<HTMLElement>node)
      )
    );
  ...
  private getPrevSibling(gridItem: GridItem): GridItem {
    const previousElement = <HTMLElement>(
      gridItem.element.previousElementSibling
    );
  ...
  private getGridItemsUnderPointer(
    ...
    const elements = this.getUnderlyingElements().filter(
      (x) => x.tagName !== "HTML"
    );
  ...
  private getGridItem(
    ...
    const widgetBinding =
      context.$data instanceof WidgetBinding
        ? context.$data
    ...
    const gridItem: GridItem = {
      ...
      select: (scrollIntoView: boolean = false) =>
        this.selectElement(gridItem, scrollIntoView),
    };

    return gridItem;
  }

  private getStylableGridItem(
    element: HTMLElement,
    ...
    const componentStyleDefinitionWrapper =
      StyleHelper.getStyleDefinitionWrappers(styleDefinitions.components);
    const match = componentStyleDefinitionWrapper.find((x) =>
      element.matches(x.selector)
    );
  ...
  private getGridItems(
    ...
    for (const element of elements.reverse()) {
      ...
      const styleableGridItem = this.getStylableGridItem(
        element,
        currentwidgetBinding
      );
